...
alpha=0.5;
% wk=alpha*randn(M,1);
wk=alpha*randn(M,1)*F0;

af=[1 -2 1].*(dT^-2)+[0 kk/m 0]+[1 0 -1]./2/dT*c/m; % af=[405,-799,395];
bf=1/m; % bf=1;
...
figure(5)
timesteps=(1:400)';
plot(timesteps,xtrue,'-.b',timesteps,xpredict,'-r');
legend('true','predict');
title('true x and predict x');
xlabel('time step');
ylabel('x (m)');

mse_filt = sqrt(sum((xtrue-xpredict).^2))